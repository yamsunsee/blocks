      ...
      switch (blockType(id)) {
        ...
        case "picked":
          if (
            ...
            hoverEffect(id, countPickedBlocks(direction), direction);
          }
          break;
        case "active":
          resetBoxShadow();
  ...
  for (const block of blocks) {
    block.onclick = function () {
      ...
      if (blockType(id) == "disable") {
        ...
        if ((blocksPicked["id"].length == 0) & (blockType(id) == "active")) {
          if (levelUpgrade > level || level == maxLevel) {
        ...
        if (blocksPicked["id"].includes(id) & (blockType(id) == "picked")) {
...
function blockType(id) {
  ...
  if (block.className == "block") {
    ...
    block.className.includes("clicked") & !block.className.includes("disable")
  ) {
  ...
  for (const block of blocks) {
    if (blockType(block.id) == "empty") {
      ...
      if (
        ...
        (blocksPicked["id"].length == 0 || blocksPicked["id"].length == 5)
      ) {
        flag = false;
      }
    }
...
function menu() {
  playBtn.onclick = function () {
    ...
    setTimeout(function () {
      ...
      creatBlocks(size, difficultyStandard(difficultyOption.value)["Disable"]);
      showTimer();
      showTaskbar();
      hover();
      click();
